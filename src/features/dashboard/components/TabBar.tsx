...
import { AlertTriangle, BellRing, LayoutDashboard, Activity } from 'lucide-react';
...
export const TabBar = memo(function TabBar({ active, counts, onSelect }: TabBarProps) {
  return (
    <div className="border-b border-slate-200 bg-white px-4 sm:px-6">
      <nav className="-mb-px flex gap-0.5" aria-label="Tabs">
        {TABS.map(({ id, label, Icon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => onSelect(id)}
              className={`group flex items-center gap-2 border-b-2 px-5 py-3.5 text-sm font-medium transition-colors focus:outline-none ${
                isActive
