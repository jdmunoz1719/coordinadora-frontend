import { incidentsApi } from '@api/incidents/incidents.api';
...
import { IncidentsFilters, IncidentsFiltersState } from './IncidentsFilters';
import { IncidentsTable } from './IncidentsTable';

export function IncidentsTabContainer() {
  useMasterData();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<Incident.ItemList[]>([]);
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);
  const [total, setTotal] = useState(0);
  const [filters, setFilters] = useState<IncidentsFiltersState>({});

  const pageRef = useRef(1);
  const limitRef = useRef(10);
  const filtersRef = useRef<IncidentsFiltersState>({});
  ...
  const load = useCallback(
    async (p = pageRef.current, l = limitRef.current, f = filtersRef.current) => {
      ...
      setError(null);
      try {
        const res = await incidentsApi.getAll({ page: p, limit: l, ...f });
        setData(res.data);
        setTotal(res.total);
        setPage(p);
        setLimit(l);
  ...
  const handleFilterChange = useCallback(
    (f: IncidentsFiltersState) => { filtersRef.current = f; load(1, limitRef.current, f); },
    [load],
  );
  ...
  return (
    <>
      ...
      {error && (
        <div className="flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-red-800">Error al cargar incidentes</p>
      ...
      <IncidentsFilters filters={filters} onFilterChange={handleFilterChange} />
