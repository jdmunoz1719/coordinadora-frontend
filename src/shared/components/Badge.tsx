...
const SEVERITY_CONFIG: Record<
  string,
  { cls: string; Icon: (p: { size?: number }) => JSX.Element }
> = {
  CRITICAL: {
    cls: "bg-red-50 text-red-700 ring-red-200",
    Icon: ({ size = 12 }) => <AlertCircle size={size} />,
  },
  HIGH: {
    cls: "bg-orange-50 text-orange-700 ring-orange-200",
    Icon: ({ size = 12 }) => <AlertTriangle size={size} />,
  },
  MEDIUM: {
...
> = {
  OPEN: {
    cls: "bg-red-50 text-red-700 ring-red-200",
    Icon: ({ size = 12 }) => <RadioTower size={size} />,
  },
  IN_PROGRESS: {
    cls: "bg-orange-50 text-orange-700 ring-orange-200",
    Icon: ({ size = 12 }) => <PlayCircle size={size} />,
  },
  ON_HOLD: {
    cls: "bg-amber-50 text-amber-700 ring-amber-200",
    Icon: ({ size = 12 }) => <PauseCircle size={size} />,
  },
  RESOLVED: {
    cls: "bg-emerald-50 text-emerald-700 ring-emerald-200",
    Icon: ({ size = 12 }) => <CheckCircle2 size={size} />,
...
export const SeverityBadge = memo(function SeverityBadge({
  ...
  if (color) {
    const Icon = (SEVERITY_CONFIG[key] ?? SEVERITY_CONFIG.INFO).Icon;
    return (
      <span
        className={base}
        style={{
